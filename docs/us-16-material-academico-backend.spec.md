...
- **Si TUTOR: el listado se filtra para devolver únicamente los materiales que el propio tutor cargó para esa materia** (`material.tutorSubject.tutorProfile.user.userId == usuarioId`) — un tutor **nunca** debe ver material cargado por otro tutor que dicte la misma materia, aunque la query base (`findActiveBySubjectId`) traiga todos los materiales de esa materia sin distinción de tutor. Este filtro es obligatorio y no debe omitirse: la falta de este filtro fue un bug detectado en implementación (ver historial de PR), donde el material de un tutor aparecía en el listado de todos los tutores que dictan esa materia.
- Devuelve lista de materiales activos (`activo = true`) de la materia, **filtrados por la relación tutor-alumno-materia (ALUMNO) o tutor-materia (TUTOR)**, cada uno con URL firmada de Supabase (expiración sugerida: 1 hora)
